...
# --- IMPORT MODUL SPAREPART ---
try:
    from spareparts import show_sparepart_menu
except ImportError:
    st.error("File spareparts.py tidak ditemukan di direktori utama!")
...
with st.sidebar:
    ...
    if not st.session_state.logged_in:
        ...
        if st.button("🔓 SIGN IN", use_container_width=True, type="primary"):
            # PROTEKSI KEYERROR AUTH
            if "auth" in st.secrets:
                if u in st.secrets["auth"] and p == str(st.secrets["auth"][u]):
        ...
        menu = st.selectbox("📂 MAIN MENU", ["Dashboard Monitor", "⚙️ Smart Inventory", "Export & Reporting", "Security Log"])
...
if menu == "Dashboard Monitor" and st.session_state.logged_in:
    ...
    # ... (Gunakan kode dashboard tiket lo yang lama di sini) ...
    st.dataframe(df, use_container_width=True, hide_index=True)

elif menu == "⚙️ Smart Inventory" and st.session_state.logged_in:
    # MEMANGGIL FUNGSI DARI SPAREPARTS.PY
    show_sparepart_menu(get_connection, get_wib_now, add_log)

elif menu == "Export & Reporting" and st.session_state.logged_in:
    st.markdown("## 📂 Export Report")
    # ... (Gunakan kode export lo yang lama) ...
    ...
    # ... (Gunakan kode quick input guest lo) ...
